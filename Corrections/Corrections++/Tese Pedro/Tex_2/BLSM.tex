...
\chapter{B-L-SM Model}\label{chapter:BLSM}

% Got this from the abstract in my BLSM paper 

\Joaoout{Here we start the our first look at BSM scenarios.}
%
In this chapter we introduce the minimal $\U{B-L}$ gauge extension of the \Joaorep{Standard Model}{SM} named\Joaoout{, the} B-L-SM \Joaoadd{(Baryon-Lepton-SM)} Model  \cite{Mohapatra:1980qe,Basso:2010hk,Basso:2011na}. 
%
\Joaorep{This is a model trough which we can explain neutrino mass generation}{In this model, we are capable of explaining the generation of neutrinos masses} via a simple see-saw mechanism\Joaoadd{. A}dditionally, by virtue of \Joaoout{the model containing} two new physical \Joaoout{particle} states, specifically\Joaoadd{,} a new Higgs like boson $\mathrm{H}^\prime$ and a $\mathrm{Z}^\prime$ gauge \Joaoadd{b}oson we can also address other \Joaorep{phenomenology}{phenomenological problems}, such as deviations in \Joaorep{electro-weak}{EW} measurements, namely the $(g-2)_\mu$ anomaly \cite{Tanabashi:2018oca}. \Joaoout{The discrepancy between the measured anomalous magnetic moment of the muon and the SM expected value}  

\Joaorep{Both the}{The} additional bosons \Joaorep{are given}{acquire} mass primarily \Joao{Primarily? Ou seja, eles têm uma contribuição de massa de outro lado? Ainda não vi mais à frente, mas se não for caso, tira a palavra primarly.} trough the spontaneous breaking of the $\U{B-L}$ symmetry that gives \Joaorep{it's}{its} name to the \Joaoadd{m}odel.
%
This unitary group originates from the promotion of \Joaorep{a}{an} accidental symmetry present in the SM, the Baryon number (B) minus the Lepton number (L) to a fundamental Abelian symmetry group. 
%
This origin for the mass of the referenced bosons means \Joaoadd{the} model is already very heavily \Joaorep{constricted}{constrained} due to long-standing direct searches \Joaorep{in}{at} the \Joaoout{Large Hadron Collide} LHC. 

T\Joaoadd{h}rough this model we can address the metastability of the \Joaoout{electroweak} EW vacuum in the SM \Joaorep{trough}{via} the new scalar. Allowing for Higgs stabilization up to the \Joaoadd{P}lank scale with a \Joaoout{the} new Higgs starting from \Joaoadd{a} few hundred \Joaoout{of} GeVs \cite{Degrassi:2012ry,Alekhin:2012py,Buttazzo:2013uya}. 

The B-L-SM \Joaoadd{framework} is particular\Joaoadd{ly} interesting in the context of the study of Grand Unified Theories (GUT) as it easily embedded into higher order symmetry groups like \Joaoout{for example} the $\mathrm{SO(10)}$ \cite{Chanowitz:1977ye,Fritzsch:1974nn,Georgi:1978fu,Georgi:1979dq,Georgi:1979ga} or $\mathrm{E}_6$  \cite{Achiman:1978vg,Gursey:1975ki,Gursey:1981kf} \Joaoadd{Lie groups}.  \Joao{Tira referências. Duas para cada grupo basta.}

The presence of a new complex singlet field, $\chi$, with a \Joaoout{(tradicional)} Higgs doublet typically results in enhanced strength of the EW phase transition\Joaoadd{,} potentially converting it into a strong first-order one\Joaoadd{. T}his would be \Joaoout{could be} detectable in the form of a gravitational wave background \cite{Barger:2008jx}. 
%
Such a analysis is of utmost importance given that it could provide a way to detect NP or exclude models without the need for a larger particle collider. \Joaoout{but instead a sensitive probe also capable of studying gravitational events.}

However\Joaoadd{,} a family-universal symmetry\Joaoadd{,} such as $\mathrm{U(1)_{B-L}}$\Joaoadd{,} being introduced without changing the SM fermion content would lead to chiral anomalies. This translates to a non conservative charged current on some channels involving the $\mathrm{U(1)_{B-L}}$. These \Joaorep{aren't}{are not} completely undesired by themselves, since \Joaorep{their result would be charge conjugation parity symmetry violation}{this allows for the presence of extra sources of $\mathcal{C}\mathcal{P}$ violation}, but \Joaorep{this}{the} inclusion at tree-level without a suppression mechanism would lead to far too much $\mathcal{CP}$ violation. 

The model also benefits from \Joaoadd{the} presence of three generations of right-handed heavy Majorana neutrinos that t\Joaoadd{h}rough the new field additions are possible in a framework free of anomalies while also \Joaorep{enabling}{allowing for} a minimal see-saw mechanism \Joaorep{to generate}{that generates} light neutrino masses unlike the SM.\cite{Yanagida:1979as,GellMann:1980vs,Mohapatra:1979ia}.
%
The mass scale of such neutrinos is established once the $\U{B-L}$ symmetry is broken. 
%
These neutrinos are of cosmological significance given their presence could imply the existence of a sterile state that can play the role of Dark Matter \Joaoout{candidate} \cite{Kaneta:2016vkq}.
%
The relatively small alteration of a $\mathbb{Z}_2$, symmetry in the neutrino sector can make these fully sterile, as seen in \cite{Okada:2010wd,Okada:2018ktp}. {\color{red} Check if the Z2 affects the Zprime, if so we must comment that it doens't allow kinetic mixing! and would alter the $a_\mu$ \Joao{Acho que não. Não deve haver alterações dos couplings com o muão, portanto a um loop não acontece nada. A dois loops, também não deve haver, pois os neutrinos não tem carga e não acoplam ao fotão, dos diagramas que aparecem na Fig. 4.7. Confirma com o Morais.}}
%
...
\Joaorep{The structure of this chapter is to first}{With this in mind, we structure this chapter in the following way. First, we} present the fundamental theoretical background on the model with a strong focus on the basic details of \Joaoadd{the} scalar and \Joaoadd{the} gauge boson mass spectra and mixing. Followed by a modern precise study of the phenomenological status of the B-L-SM model t\Joaoadd{h}rough \Joao{Tu esqueçes dos h's em ``Through'', tem cuidado, trough é outra coisa.} a layered algorithm that will be discussed preceding the results. \Joaorep{Through}{With} this algorithm we provide a numerical analysis that tests the relevant phenomenological constraints in direct and \Joaorep{electroweak}{EW} observables. Followed by this study\Joaoadd{,} we table of\Joaoadd{f} a few representative benchmark points \Joaorep{to be possibly tested in by experiments}{that can be tested by experiments}. 

{ \color{blue} perhaps I should include this on the start of the chapter  \Joao{Fica bem aqui.}}

\section{Formulating the model}\label{section:Form_Model}

Essentially, the minimal B-L-SM is a BSM framework containing only three new ingredients, a new gauge interaction given the new symmetry group, three generations of right handed neutrinos and a complex scalar field $\chi$. The first of these, is motivated by the aforementioned GUT scenarios\Joaoout{, as seen in the Refs, [20], [21], [22], [23], [24], [25], [26], [27] here you can see that the B-L-SM can be introduced in a large number of groups, like $E_6$ and $\mathrm{SO}(10)$.} \Joaorep{Secondly, as mentioned,}{while} a new sector of additional three \Joaoadd{$\mathrm{U(1)_{B-L}}$} charged Majorana neutrinos is essential for anomaly cancellation. \Joaoout{and addresses many concerns of the SM.}

%\cite{Chanowitz:1977ye,Fritzsch:1974nn,Georgi:1978fu,Georgi:1979dq,Georgi:1979ga,Achiman:1978vg,Gursey:1975ki,Gursey:1981kf}


Finally, the SM-like Higgs doublet, $H$, does not carry neither baryon nor lepton number, \Joaoadd{in} this way it does not participate in the breaking of \Joaoadd{$\mathrm{U(1)_{B-L}}$}. It is then necessary to introduce a new scalar singlet field, $\chi$, solely charged under \Joaoadd{$\mathrm{U(1)_{B-L}}$}, to perform the breaking of the \Joaoadd{$\mathrm{B-L}$} symmetry.

The particle content and related charges of the minimal \Joaoadd{$\mathrm{U(1)_{B-L}}$} extension of the SM are shown in \Joaorep{the table}{in Tab.~\ref{tab:charges}}. Note these are similar to the SM as to be expected. 
%
\begin{table}[H]
\centering
...
\hline
  & $q_L$  & $u_R$ & $d_R$ & $l_L$  & $e_R$ & $\nu_R$  &  $H$  & $\chi$  \\ \hline
 \Joaoadd{$\mathrm{SU(3)_C}$}& \textbf{3} & \textbf{3} & \textbf{3} & \textbf{1} & \textbf{1} & \textbf{1} & \textbf{1}  & \textbf{1}  \\
 \Joaoadd{$\mathrm{SU(2)_L}$}& \textbf{2}  & \textbf{1} & \textbf{1} & \textbf{2} & \textbf{1} & \textbf{1} & \textbf{2}  & \textbf{1} \\
\Joaoadd{$\mathrm{U(1)_Y}$} & $1/6$ & $2/3$  & -$1/3$  & -$1/2$ & -1 & 0 & $1/2$ & 0 \\
\Joaoadd{$\mathrm{U(1)_{B-L}}$} & $1/3$ & $1/3$ & $1/3$  & -1  & -1 &-1  & 0 & 2  \\ \hline 
\end{tabular}
\caption{Quantum fields and their respective quantum numbers in the minimal B-L-SM extension. The last two lines represent the weak and $B-L$ hypercharges. \Joao{Adicionei negrito. Tirei as fracções, acho que assim fica melhor. Se não achares, podes voltar a por de volta}}
\label{tab:charges}
\end{table} 

\subsubsection{Scalar sector}

\Joaorep{Given the information seen above}{With the information from Tab.~\ref{tab:charges}}, we can begin examining the new Lagrangian terms. Starting by the scalar potential, which now depends on two fields\Joaoout{ as seen in}, 
%
\begin{equation}
\label{eq:potential}
...
\end{equation}
%
\Joaorep{Being,}{where} $\lambda_i$ \Joaoadd{are} the scalar couplings \Joaoadd{, $\mu_1^2$ a quadratic term for $H$ and $\mu_2^2$ a quadratic term for the singlet $\chi$}. This potential must lead to \Joaoadd{a} stable vacuum state, \Joaorep{for this}{which means that} the scalar potential must be bounded from below (BFB), as to ensure a global minima.  Studying the potential on \Joaoadd{Eq}.\,\eqref{eq:potential} we deduce the conditions,
\begin{equation}
...
\end{equation}
%
\Joaoadd{where we note that the parameters $v$ and $x$ are VEV's associated with the $H$ field and the $\chi$ field, respectively.} In these equations we can see \Joaoadd{that} $h$ and $h^\prime$ represent\Joaoout{ing} the radial quantum fluctuations around the minimum of the potential. These will constitute the physical degrees of freedom associated \Joaorep{to the}{with} $H$ and $H^\prime$. There are also four Goldstone directions denoted as $\omega_1$, $\omega_2$, $z$ and $z^\prime$ which are absorbed into longitudinal modes of the $W^\pm$, $Z$ and $Z^\prime$ gauge bosons once \Joaoout{spontaneous symmetry breaking} SSB takes place. After SSB the associated VEVs take the form, 
%
\begin{equation}
...
From here we can solve the tadpole equations in relation to each of the VEVs as to ensure non-zero VEV\Joaoadd{. W}e arrive at,
%
\begin{equation}
	v^2 = \dfrac{-\lambda_2 \mu_1^2 + \dfrac{\lambda_3}{2}\mu_2^2}{\lambda_1 \lambda_2 - \dfrac{1}{4}\lambda_3^2} > 0
	\qquad
	\text{and}
	\qquad
	x^2 = \dfrac{-\lambda_1 \mu_2^2 + \dfrac{\lambda_3}{2}\mu_1^2}{\lambda_1 \lambda_2 - \dfrac{1}{4}\lambda_3^2} > 0 
	\label{eq:extremum}
\end{equation}
%
which, when simplified with the \Joaorep{bound from bellow}{BFB} conditions yield a simpler set of equations,
%
\begin{equation}
...
\end{equation}
%
Note that although $\lambda_1$ and $\lambda_2$ must be positive to ensure the correct conical shape \Joaorep{to}{of} the potential, no such conditions exist for the sign of $\lambda_3$ , $\mu_1$, and $\mu_2$. However\Joaoadd{,} observing \Joaorep{equation}{Eq.\,\eqref{eq:sols},} we can infer that only some combinations of signs are impossible, 
%
\begin{table}[H]
	\begin{center}
		\begin{tabular}{ccccc}
			...
			\hline
		\end{tabular} 
		\caption{Possible \Joaoadd{s}igns of the potential parameters in \Joaoadd{Eq.}\,\eqref{eq:potential}. 
\Joaorep{While the}{The} \checkmark\,symbol indicates the existence of solutions for tadpole conditions \Joaoadd{Eq.}\,\eqref{eq:sols}, \Joaoadd{while} the \xmark\,indicates unstable configurations.}
		\label{tab:signs}  
	\end{center}
\end{table} 
%
For our numerical analysis we decided to leave the sign of $\lambda_3$ positive, choosing a configuration where both $\mu$ parameters are negative. This \Joaorep{doesn't}{does not} directly translate to any real physical consequence.  
%
\Joaorep{These}{With these} conditions now established we proceed to investigate the physical states of \Joaoadd{the} B-L-SM scalar sector. \Joaorep{By first, taking the Hessian matrix evaluated at the vacuum value,}{At the vacuum, we evaluate the Hessian matrix as}
%
\begin{equation}
\mathbf{M}^2 =
\begin{pmatrix}
...
\end{equation}
% 
Moving this matrix to \Joaorep{it's}{its} physical mass eigenbase, we obtain the following eigenvalues
%
\begin{equation}
...
\end{equation}
%
\Joaorep{Being that, the rotation matrix is written as,}{where \textbf{O} can be parameterized by a single mixing angle $\alpha_h$,}
%
\begin{equation}
	\mathbf{O} = 
	\begin{pmatrix}
...
\end{equation} 
\Joao{Introduziste um $v^\prime$. O que é isto? Acho que te enganaste, é suposto ser $x$. Acho também que os sinais do denominador estão trocados mas isso já não tenho a certeza.} \Joaorep{Although consider it is worth presenting the case of approximate decoupling where, $v/x\ll 1$. In this case scalar masses and mixing angle become particularly simple,}{It is particularly interesting to analyse the scenario where the scalar fields approximately decouple, that is, in the limit $v/x\ll 1$. In these circumstances, the scalar masses and the mixing angle become rather simple,}
\begin{equation}
...
\end{equation}
%
We will see in the context of our numerical results that for \Joaorep{our}{a} phenomenologically consistent mass scale\Joaoadd{,} these equations serve a\Joaoadd{s a} valid approximation for most of the  points. 
...
\begin{equation}
\mathcal{L}_{\Joaoadd{\mathrm{U(1)'s}}} =  \left| D_\mu H \right|^2 + \left| D_\mu \chi \right|^2 -\dfrac{1}{4} F_{\mu \nu} F^{\mu \nu} -\dfrac{1}{4} F^\prime_{\mu \nu} F^{\prime \mu \nu} -\dfrac{1}{2} \kappa F_{\mu \nu} F^{\prime \mu \nu}
\label{eq:Lu1}
\end{equation}
where $F^{\mu \nu}$ and $F^{\prime \mu \nu}$ are the standard field strength tensors, respectively for the \Joaoout{hypercharge} $\U{Y}$ and \Joaoout{B minus L} $\U{B-L}$ \Joaoadd{Abelian groups}, 
\begin{equation}
	F_{\mu \nu} = \partial_\mu A_\nu - \partial_\nu A_\mu 
	\qquad
	\text{and}
	\qquad
	 ...
	 \label{eq:Fmn}
\end{equation}
written in terms of the gauge fields $A_\mu$ and $A_\mu^\prime$, respectively. Given that this is a model with two \Joaoadd{u}nitary groups, without a parity symmetry ($\mathbb{Z}_2$) to prevent it, we must consider the possible mixing in between them. In this work\Joaoadd{,} we \Joaorep{parametrized}{parameterized} this mixing trough a parameter $\kappa$.

The Abelian part of the covariant derivative in \Joaorep{equation}{Eq.\,\eqref{eq:Lu1}} is given by
\begin{equation}
	D_\mu \supset i g_1 Y A_\mu + i g_1^\prime Y_{\rm B-L} A_\mu^\prime\,,
\end{equation} 
% 
with $g_1$ and $g_1^\prime$ being the $\U{Y}$ and $\U{B-L}$ \Joaoout{the} gauge couplings\Joaoadd{.} \Joaorep{with the}{The} $Y$ and \Joaoadd{$\mathrm{B-L}$} charges are specified in Tab.~\ref{tab:charges}. \Joaorep{However it}{It} is convenient to rewrite the gauge kinetic terms in the canonical form, i.e.
%
\begin{equation}
	F_{\mu \nu} F^{\mu \nu} + F^\prime_{\mu \nu} F^{\prime \mu \nu} + 2 \kappa F_{\mu \nu} F^{\prime \mu \nu} \to B_{\mu \nu} B^{\mu \nu} + B^\prime_{\mu \nu} B^{\prime \mu \nu}\,.
	\label{eq:AtoB}
...
\begin{equation}
	...
	\label{eq:new-g-simp}
\end{equation}
which is the standard convention in the literature. Note \Joaoadd{that} this definition is merely to \Joaorep{simply}{simplify} the equations and has no physical impact. We will later see that this kinetic mixing is a desired feature and why stabilizing it with a $\mathbb{Z}_2$ symmetry would be detrimental in terms of depth. The resulting mixing between the neutral gauge fields including $Z^\prime$ can be represented as follows
%
\begin{equation}
\begin{aligned}
\begin{pmatrix}
...
\begin{equation}
\sin(2 \theta^\prime_W) = \frac{2 g_{YB} \sqrt{g^2 + g_{Y}^2}}{\sqrt{(g_{YB}^2 + 16 \Bigg(\dfrac{x}{v}\Bigg)^2 g_{B-L}^2 - g^2 - g_{Y}^2)^2 + 4 g_{YB}^2 (g^2 + g_{Y}^2)} }\,,
\label{eq:theta-p-full}
\end{equation}
in terms of $g$ and $g_{Y}$ being the \Joaoadd{$\mathrm{SU(2)_{L}}$} and \Joaoadd{$\mathrm{U_{Y}(1)}$} gauge couplings, respectively. In the physically relevant limit, $v/x \ll 1$, the above expression greatly simplifies leading to
\begin{equation}
...
\end{equation}
%
up to $(v/x)^3$ corrections. \Joao{Não seria $\sin(2\theta_W^\prime)$?} In the limit of no kinetic mixing, i.e. $g_{YB} \to 0$, there is no mixture of $Z^\prime$ and SM gauge bosons. 

Note \Joaoadd{that} the kinetic mixing parameter $\theta_W^\prime$ has rather stringent constraints from $Z$ pole experiments both at the Large Electron-Positron Collider (LEP) and the Stanford Linear Collider (SLC), restricting its value to be smaller than $10^{-3}$ \Joaoout{approximately} \Joao{Refs para isto}, which we set as an upper bound in our numerical analysis. Expanding the kinetic terms $\left| D_\mu H \right|^2 + \left| D_\mu \chi \right|^2$ around the vacuum one can extract the following mass matrix for \Joaoadd{the} vector bosons
\begin{equation}\label{eq:mass_matrix_vectors}
	m_V^2 =
	\dfrac{v^2}{4}
	\begin{pmatrix}
	g^2 \;\;&\;\; 0 \;\;&\;\; 0 \;\;&\;\; 0 \;\;&\;\; 0 \\
...
\end{equation}
Investigating the nature of this set of eigenvalues allows us to understand the see-saw \Joaoadd{process}. The mean of these values being always equal to $|B|$, if one value goes up, another goes down, like a see-saw. \Joaorep{Note $B$, generally is the Majorana mass terms, and is generally very large in comparison to the cross interaction terms.}{$B$ is set to be proportional to Majorana mass terms, orders of magnitude higher than the cross-terms $A$.} Given this\Joaoadd{,} the smaller \Joaorep{eigenstate}{eigenvalue} \Joaorep{to be approximate,}{is} 
\begin{equation}
...
\begin{aligned}
\mathcal{L}_f = 
-Y_u^{ij} \Joaoadd{\bar{q}_{\rm L i}} u_{\rm R j} \widetilde{H} 
-Y_d^{ij} \Joaoadd{\bar{q}_{\rm L i}} d_{\rm R j} H
-Y_e^{ij} \Joaoadd{\bar{\ell }_{\rm L i}} e_{\rm R j} H
- Y_\nu^{ij} \Joaoadd{\bar{\ell}_{\rm L i}} \nu_{\rm R j} \widetilde{H}
	-\dfrac{1}{2} Y_\chi^{ij} \Joaoadd{\bar{\nu}_{\rm R i}^c} \nu_{\rm R j} \chi + {\rm \Joaoadd{\mathrm{H.c.}}}
\end{aligned}
\label{eq:Yuk}
\end{equation}
%
...
Before we begin this section \Joaorep{consider that our colleges in a recent work tested the state of the art at the LHC for low mass $Z^\prime$ boson}{,we would like to point-out recent work done by our colleges where a comprehensive study for the $Z^\prime$ at the LHC is performed} \cite{Deppisch:2019ldi}. In particular from $0.2$ \Joaoadd{GeV} to $200$ \Joaoadd{GeV}. As for slightly heavier $Z^\prime$ masses beyond $m_{Z^\prime} \gtrsim 100~\mathrm{GeV}$, the combined effect of the electroweak precision observables and the ATLAS searches for Drell-Yan $Z^\prime$ production decaying into di-leptons, i.e.~$pp \to Z^\prime \to ee,\mu \mu$ \cite{Aaboud:2017buh}, is also finely investigated.
%
We then endeavoured to achieve a complementary study where we investigated the case of very heavy $Z^\prime$ bosons. 

\Joaorep{Our goal was to discover if it was still possible to, with such a heavy $Z^\prime$ boson, limited by LHC, with such a heavily constrained kinetic mixing, to have any significant phenomenological impact asides from simply a yet to be observed boson.}{Our goal is to see if the case of an heavy $Z^\prime$, whose kinect mixing is highly constrained by LHC experimental results, can provide additional phenomenological implications besides the addition of a new unobserved vector boson.} This was chiefly done by the investigation of the  $\left(g-2\right)_\mu$ anomaly.
%
We examine the relations that this anomaly has with the parameter space, such as gauge couplings, as well as the extra scalar mass. The $\left( g-2 \right)_\mu$ anomaly refers to the discrepancy between the measured anomalous magnetic moment of the muon,
...
\end{equation}
with numbers in brackets denoting experimental and theoretical errors, respectively. This represents a \Joaorep{tension}{deviation} of $3.5$ standard deviations from the combined $1 \sigma$ error and is calling for new physics effects beyond the SM theory. 
...
\subsection{The scanning apparatus}\label{subsection:Scanning_apparatus}

The numerical data presented in this section \Joaoout{of our work} was generated \Joaorep{thorough}{via} a large chain of nested scripts. These were created as a possible first generation of a scanning framework for generic phenomenological models. This machinery was adapted to the 3HDM numerical scan so this introduction will be glossed over in the 3HDM section, as much remained the same. 

\Joaorep{The scripts were a mixture of Linux bash and Python 3 scripts}{The underlying code is a mixture of bash and Python3 scripts}, and utilize\Joaoadd{s} \Joaoout{the} \texttt{SPheno 4.0.3} \cite{Porod:2003um,Porod:2011nf},  \texttt{SARAH 4.13.0} \cite{Staub:2008uz,Staub:2013tta}, \texttt{HiggsBounds 4.3.1} \cite{Bechtle:2013wla}, \texttt{HiggsSignals 1.4.0} \cite{Bechtle:2013xfa} and \texttt{MadGraph5\_aMC@NLO 2.6.2} \cite{Alwall:2014hca} programs/packages. \Joao{Uma referência para cada, tira uma do Spheno e outra do SARAH}

These scripts generate a Monte-Carlo type scan through a desired parameter space. Unless introduced, all non-relevant physical constants and parameters are defined in a way as to keep the observed gauge, lepton and quark structure consistent with the SM. Skipping a bit ahead, as \Joaorep{a}{an} example, for the B-L-SM scan our \Joaoout{scanning} routine randomly samples parameter space points according to the ranges in Tab.~\ref{tab:scan} while keeping \Joaorep{things}{variables} like \Joaoadd{the} Higgs doublet VEV and Weinberg angle to reproduce the correct 
$W$ and $Z$ structure.

Given the randomness in our scan, we can reach unphysical or nonsensical regions, that contain objects like tachyonic scalar masses\Joaoadd{,} un-renormalizable quantities, divergent radiative corrections etc. These points must be rejected before even considering experimental constraints. This is done by \texttt{SPheno}, rejecting any point generated with unphysical parameters. 
...
%
The latter is made automatically t\Joaoadd{h}rough the package \texttt{HiggsBounds 4.3.1} that shall be used to apply a $95\%$ C.L. exclusion limit cut on a new scalar particle, $h_2$, while \texttt{HiggsSignals 1.4.0} is used to calculate and later check, through a $\chi^2$ distribution, the probability for consistency with the observed Higgs boson signal data. 
% 
To calculate these variables \texttt{HiggsBounds 4.3.1} and \texttt{HiggsSignals 1.4.0} are provided \Joaoadd{with} all scalar masses, total decay widths, Higgs decay branching ratios as well as the SM-normalized effective Higgs couplings to fermions and bosons squared (that are needed for analysis of the Higgs boson production cross sections). \Joaoout{For details about this calculation, see Ref.~\cite{Bechtle:2013wla}.}
%
...
\texttt{SPheno} generates all relevant data from any given parameter space point. \Joao{Não estou a perceber a frase anterior, tens de a reescrever.} \texttt{SPheno} is a particle spectrum generator code written in Fortran 90. \Joaorep{It's}{Its} emphasis on easy generalisability and speed made it a natural part of our numerical analysis. It takes information about our models Lagrangian, such as fields, charges and fundamental symmetries, and creates a executable file capable of quickly generating a spectrum file with all details regarding mass, decay and flavour observables information in the standardized SUSY Les-Houches accord format. All generated spectrums are processed and stored. 
%
...
%
The lepton anomalous magnetic moments $\left( g-2 \right)_\ell /2 \equiv a_\ell$ are calculated in \texttt{SPheno} at one-loop order. In the B-L-SM, \Joaoout{new physics} NP contributions to $a_\mu$, denoted as $\Delta a_\mu^{\textrm{NP}}$ in what follows, can emerge from the diagrams containing $Z^\prime$ or $h_2$ propagators, as \Joaorep{a example consider}{seen in} Fig.~\ref{fig:g-2}. \Joao{É usual mencionar as figuras pela ordem que elas aparecem, portanto esta figura devia estar em primeiro lugar.} 
...
\end{table}
%  
\Joaoout{Keeping the remaining free parameters of the model to be in agreement with the Standard Model.  }
%
...
As stated \Joaoadd{before,} we confront the surviving scenarios, black points in Fig.~\ref{fig:STU}, with collider bounds. In particular, $95\%$ C.L.~exclusion limits on a new scalar particle and check for consistency with the observed Higgs boson at $3 \sigma$. \Joao{Chegaste a definir o acrónimo de C.L.? Eu já não sei. E porque 3$\sigma$? Por que não até 5$\sigma$? Parece um pouco arbitrário.}
%
...
%
From here we move onto the third layer of phenomenological tests \Joaorep{we have studied the viability of the surviving scenarios}{where we look at the viability of certain points} from the perspective of direct collider searches for a new $Z^\prime$ gauge boson at the most recent collider experiments.
%
...
\end{figure}	
%%%%%%%%%%%%%%%%%%%%%%%%%
Since the couplings of a new scalar $h_2$ to the SM fermions are suppressed by a factor of $\sin \alpha_h$, which we find to be always smaller than $0.08$ as can be seen in the bottom panel of Fig.~\ref{fig:Plots4}, the right diagram in Fig.~\ref{fig:g-2}, which scales as $\Delta a_\mu^{h_2} \propto \Joaoadd{(m_\mu^2/m_{h_2}^2)}\left(y_\mu \sin \alpha_h\right)^2$ with $\sin^2 \alpha_h < 0.0064$ and $y_\mu = Y_e^{22}$, \Joaorep{provides}{providing} sub-leading contributions to $\Delta a_{\mu}$. \Joao{Evita usar fracções no texto, fica difícil de ver} Furthermore, as we show in the top-left panel of Fig.~\ref{fig:Plots4} the new scalar boson mass, which we have found to satisfy $m_{h_2} \gtrsim 380~\ro{GeV}$, is not light enough to compensate the smallness of the scalar mixing angle. \Joao{Não estou a perceber. Explica me depois} Conversely, \Joaoout{and recalling that all fermions in the B-L-SM transform non-trivially under $\U{B-L}$,} the new $Z^\prime$ boson can have sizeable couplings to fermions via gauge interactions proportional to $g_{\rm B-L}$, \Joaoadd{as seen in the bottom pannel of Fig.~\ref{fig:Plots3}} Therefore, the left diagram in Fig.~\ref{fig:g-2} provides the leading contribution to the $\left(g-2\right)_\mu$.\Joaoout{in the model under consideration.}
%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[!htb]
	\centering
...
\end{equation}
\Joao{Em algumas das tuas equações tu tens tfrac, eu tenho estado a tirar isso. Deixa as fracções demasiado pequenas} to \Joaoout{the} second order in $v/x$-expansion. If $v/x \ll 1$, corresponding to the darker shades of the color scale in the top-right panel of Fig.~\ref{fig:Plots4}, we can further approximate
%
\begin{equation}
...
\end{equation}
%
Similarly, for the yellow band, which corresponds to the region where $\Delta a_{\mu}^{\rm NP}$ is maximized (see top-left panel of Fig.~\ref{fig:Plots1} \Joao{Não exite nenhuma banda amarela nestes plots. Acho que estas a falar dos plots da Fig. 4.3, mass esses não fazem sentido pois não estão em função do $\Delta a_\mu$}), a large value of the $\U{B-L}$ gauge coupling also allows one to simplify Eq.~\eqref{eq:ZpContribution} reducing it to the form of Eq.~\eqref{eq:amu-simple}. This is \Joaoout{in fact} what we have observed and, for the yellow band region, we see in the bottom panel of Fig.~\ref{fig:Plots3} that $g_{\ro{B-L}} \simeq 3$. A sizeable value of $g_{\ro{B-L}}$ is indeed what is contributing to the enhancement of $\Delta a_{\mu}^{\rm NP}$, in particular, for the red region in both panels of Fig.~\ref{fig:Plots1}. We show in the third and fourth lines of Tab.~\ref{tab:bench} the two benchmark points that better reproduce the muon anomalous magnetic moment represented by two black dots \Joao{novamente, acho que devias mudar para duas cores distintas, pois são dois benchmarks diferentes.} in
...
\begin{figure}[!htb]
	...
	\caption{The same as in Fig.~\ref{fig:Plots4} but with the colour scale representing the gauge-mixing parameters $g_{\ro{YB}}$ (top-left) \Joaoout{and}\Joaoadd{,} $\theta_{W}^{\prime}$ (top-right), and the $\U{B-L}$ gauge coupling (bottom).}
	\label{fig:Plots3}
\end{figure}	
%%%%%%%%%%%%%%%%%%%%%%%%%
...
To understand this behaviour, let us also look to Fig.~\ref{fig:Plots3} (\Joaorep{top-right}{top-left} panel) where we see that the kinetic-mixing gauge coupling $g_{\ro{YB}}$ is typically very small apart from two green bands where it can become of order $\mathcal{O}(1)$.
...
\begin{equation}
    ...
    \label{eq:agZ}
\end{equation}
\Joaoadd{where $T_7$ is a loop-integral, described in appendix \ref{app:T7}}. \Joaorep{where}{The parameters} $g_{\ro{L,R}}^{ttZ^\prime}$, calculated in \texttt{SARAH}, are the left- and right-chirality projections of the $Z^\prime$ coupling to top-quarks, given by
\begin{equation}
\begin{aligned}
...
%
Done \Joaorep{through}{by} exploring \Joaoadd{the} B-L-SM potential for the observed $(g-2)_{\mu}$ anomaly in the regions of the model parameter space that are consistent with direct searches and electroweak precision observables.

As one of the main results of our analysis, we have found phenomenologically consistent \Joaoout{model} parameter space regions that simultaneously fit the exclusion limits from direct $Z^\prime$ searches and can explain the muon $(g-2)_{\mu}$ anomaly. 
%
...
\Joao{Acho que deves detalhar mais um pouco aqui. Mais concretamente, deves indicar em que zonas do espaço de parâmetros é que obtens as melhores contribuições para o $\Delta a_\mu$, ou seja, para que valores dos vev's e do gauge coupling do $Z^\prime$ te dá os melhores resultados.}
