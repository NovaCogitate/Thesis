...
As stated in chapter \ref{Chap:Introduction}, it is hard to question the validity of the SM as a successful, at least approximate, framework with whom to describe the phenomenology of particle physics up to the largest energy scales probed by collider measurements so far\Joaoadd{. A}lthough some inconsistencies remain and must be addressed.  

What we call the SM is the conjugation of several theories, quantum chromodynamics (QCD), Higgs sector and \Joaoout{electroweak} \Joaoadd{the} EW theory \Joao{Já definiste o acrónimo na introdução}. The last piece\Joaoout{, the EW theory} was introduced in the nineteen sixties by Glashow, Salam and Weinberg \cite{SALAM1964168} and since it has been extensively tested both in contemporary direct searches for new physics and indirect probes via e.g. flavour anomalies and precise electroweak parameter measurements in proton-electron collisions \cite{Gr_newald_2008}. It awarded the authors the 1979 Noble prize of physics \cite{NobelPrize:2009-Physics}.  
%
...
Physicists took this idea and were led to the fundamental question behind the SM, is it possible that upon imposing to a given Lagrangian the invariance under a certain group of symmetries to reach a given form for its the dynamics? 
%
...
%
Recall \Joaoadd{that} QFT treats particles as excited states of their respective quantum fields, which are more fundamental than the particles. 
%
...
%
\Joaorep{Of which each interaction can be visually represented by \Joaoout{a} Feynman diagrams according to perturbation theory.}{Generally speaking, an interacting QFT can not be solved analytically, requiring the use of perturbative methods. In this mindset, one employs the use of Feynman diagrams to better visualize interactions at any given order in perturbation theory.} 
...
%
This chapter will focus on how the mass of particles is generate\Joaoadd{d} via the Higgs mechanism.
%
...
From these fields, the particle spectrum of the SM is composed by, the gauge bosons, $W^\pm$ and $Z$ bosons\Joaoadd{,} mediators of the weak interactions. \Joaoadd{T}he photon $\gamma$, the electromagnetic interaction messenger and the strong force mediators, the gluons, $G$. \Joaorep{as well as}{Additionally, the model is also composed} by the matter particles, the fermions, composed by \Joaoout{the} quarks and leptons. A spin-0 scalar also emerges known as the Higgs Boson, $h$. 
...
%
Similarly, there are 6 types of leptons, the 3 charged leptons, electron, muon and tau, and the associated neutrinos. These are represented in different manners, being that the quarks are represented by \Joaoout{the letters} $(u,d,c,s,t,b)$ while leptons as $(e,\nu_{e},\mu,\nu_{\mu},\tau,\nu_{\tau})$, where $\nu$ denotes the neutrinos. 

Fermions are half integer spin particles \Joaoadd{where} half of which have electrical charge (except the neutrinos).  While quarks interact via the weak, electromagnetic and strong forces, the charged leptons only feel the electromagnetic and weak forces and the neutrinos are weakly interacting.  
%
...
\end{equation}
%
\Joaoadd{N}ote that the quarks form triplets of $\mathrm{SU(3)_C}$ whereas leptons are colour singlets, meaning that only quarks interact strongly. The Higgs boson also emerges from an $\mathrm{SU(2)_L}$ doublet with the form,
%
\begin{equation}
H=\begin{pmatrix}
\phi^1 + \; i \; \phi^2 \\
...
%
where we see the four components that correspond to the respective degrees of freedom of the Higgs Field. \Joao{Adicionei o vector com o $\phi^+$ e $\phi^0$ para ficar mais consistente com a tabela 2.2}
% 
After the process of \Joaoadd{spontaneous symmetry breaking} \Joaoadd{(}SSB\Joaoadd{)} of the $\mathrm{SU(2)_L} \times \U{Y}$ group\Joaoadd{,} the charges \Joaoadd{read as}
%
\begin{table}[H]
...
The full set of SM fields and respective quantum numbers are are shown in the tables \ref{table1} and \ref{table2}. These show us the representations of these fields, e.g. if a field $F$ has a quantum number of 3 under $\mathrm{SU(2)_L}$ then he would be a triplet of of $\mathrm{SU(2)_L}$, \ $F^a = (F^1,F^2,F^3)$, where $a$ is \Joaoadd{an index in} the adjoint representation of the group.   
%
\begin{table}[H]
\centering
\caption{Gauge and Scalar fields in the SM. \Joao{Nestas tabelas tens os grupos em itálico, eu alterei com \texttt{\char`\\ mathrm}}}
\label{table1}
...
\begin{table}[H]
\centering
\caption{Fermion fields in the SM. \Joao{Tirei a definição de $Q$ e $L$, já tens definido na eq. 2.2.}}
\label{table2}
\begin{tabular}{@{}cccccc@{}}
  \hline	
...
\end{equation}
%
where, $\tilde{H}=i\sigma_2 H$ and H.c. stands for Hermitian conjugate, also the terms $Y^{e,u,d}$ stand\Joaoout{s} for the Yukawa matrices, these are generic $3\times3$ with complex and \Joaorep{non-dimensional}{adimensional} matrix elements. Note that all indices seen in Eqs.\,(\ref{eq:KinSM}), (\ref{eq:PotentialSM}) and (\ref{eq:YukawaSM}), ($j,i$) are summed over. 
...
\end{equation} 
called the \Joaorep{electroweak VEV}{EW-VEV}, \Joaoadd{Its value} is \Joaoout{experimentally} measured to be $v \approx 246$ GeV. 
%
...
Given that now the $\mathrm{SU}(2)_{\mathrm{L}} \times \mathrm{U}(1)_{\mathrm{Y}}$ symmetry is broken down to $U(1)_{\mathrm{Q}}$ we jump from a scenario where there were four generators, which are $T^{1,2,3}$ and $Y$, to\Joaoout{, after the breaking,} having solely one unbroken combination that is $Q =  (T^3 + \Joaoadd{Y}/2)$ associated to the electric charge.
%
This means that in total we will have three broken generators, thus, from the Goldstone theorem, there would have to be created three massless particles. { \color{red} Do I need citation? }\Joao{Acho que não é necessário. Eu na minha tese não coloquei.}

These Goldstones modes however can be parameterized as phases in field space and can be ``rotated away" in the physical basis, leaving us with a single physical massive scalar, the Higgs boson. Note that, with this transformation we are removing three scalar degrees of freedom.  However, they cannot just disappear from the theory and will be absorbed by the massive gauge bosons. 
%
...
\end{equation}
%
\Joaoout{Thus showing the massless photon along with a massive Z boson with mass $M_Z= \frac{1}{2} \Joaoadd{v} \sqrt{g^2 + g^{\prime 2}} $. }
%
...
The reason for such a striking difference is that in the SM the charged currents occur at tree level, while FCNCs are forbidden at tree level and only arise starting at one loop order. Note the lack of neutral couplings between the up and down families in \Joaoadd{Eq.~\eqref{LagFermFCCCs}}. The relative complexity of these processes can be easily seen in Fig \ref{fig:Flavour_D_1}\Joaoadd{.}
%
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{Stolen.png}
	\caption{Representative tree level charged current diagram (left) and a loop induced FCNC diagram (right). \Joao{Tendo em conta que o nome do ficheiro é \texttt{Stolen.png}, acho que tiraste a imagem de algum lado. Põe aqui a referência}.}
...
%
A interesting result of this mechanism would be that that there is no flavour violation, if all the quark masses are the same. { \color{red} Relevant? } \Joao{Acho que isto é mais uma curiosidade, portanto podes tirar.}
...
Thanks to these large experiments we have many new observables in flavour physics, e.g. the branching ratios not coinciding with the SM prediction \cite{CasteloBranco2014}, observed Lepton F\Joaoadd{l}avour Violation in tree and loop levels \cite{Graverini2019}.
%
...
However, one of the favoured ways to search for NP is trough FCNCs\Joaoadd{. T}his is because \Joaoadd{,} as mentioned \Joaoadd{before,} there are no FCNCs at tree-level in the SM i.e. the gluon, Z and Higgs are strictly flavour conserving as we see in Fig.\,\ref{fig:QuarkCKM}.  
%
...
\noindent\begin{minipage}{.35\textwidth}
	\begin{figure}[H]
		\label{fig:contactNP}
		\centering
		\includegraphics[width=0.75\textwidth]{My_First_Diagram.png}
...
\bigbreak
%
To explain $b \rightarrow s \mu \mu$ transitions you would need a $\Lambda_{NP} \approx 3 \ \text{TeV}$ while for $b \rightarrow c \tau \nu$ you would need a $\Lambda_{NP} \approx 30\ \text{TeV}$. This is a strong indicator that some components are missing in our formulation like a new mediator for gauge interactions. \Joaorep{And the}The{} advantage of this scale \Joaorep{is it}{lies} almost certainly in most BSM scenarios, avoiding most experimental constraints.
...
contributions (middle and right).\Joao{Ref. Põe as images um bocadinho maior. Não se vê muito bem as labels. A mesma coisa para a fig. 2.5} }
...
%
The prediction of LFU can be tested experimentally, also t\Joaoadd{h}rough flavour physics, by theoretically clean observables such as the ratio\Joaoout{s of these flavour observables,} 
%
\begin{equation}
