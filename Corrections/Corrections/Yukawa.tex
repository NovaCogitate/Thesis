...
\end{equation} 
%
\Joaoadd{As} the Higgs field settles into the EW-VEV (see Eq.\,(\ref{shame}))\Joaoadd{,} mass terms for the quarks and leptons are generated. 
%
...
To reach the physical states from the weak eigenbasis \Joaorep{you}{we} must diagonalize the Yukawa matrices. This is done through bi-unitary transformations. 
% 
We can write these transformation\Joaoadd{s} under the form,
%
\begin{equation}
\label{YukawaMasses} 
...
%
For \Joaorep{simplicities}{simplicity} sake we assume that the leptonic Yukawa couplings matrix $Y^{e}$ is flavour diagonal i.e. a diagonal matrix needing not to be transformed. This makes only the quarks to be relevant to our discussion\Joaoout{s}.  

Naturally, we can invert \Joaoadd{Eq.\,\eqref{YukawaMasses}}, returning, 
\begin{align}
...
\end{align}
%
Considering the Higgs mechanism, we can see this change creates mass terms for physical quark fields by replacing the result of \Joaoadd{Eq.\,\eqref{eq:YukawaBiUni}} in the Yukawa portion of the Lagrangian (\Joaoadd{Eq.\,\eqref{eq:YukawaSM}}) \Joaoout{.}
%
\begin{gather}
...
%
If you then take into account the real masses e.g. for the leptons, the tau mass is in the GeV range while the electron's is in the 0.1 MeV range. This translate\Joaoadd{s} to very different couplings for each flavour. 
%
...
%
The coupling of the fermions to the gauge fields changes by virtue of the fact that only left handed quarks are \Joaoadd{$\mathrm{SU(2)_L}$} doublets. If we expand the up and down quark fields on the kinetic portion of the Lagrangian,
%
\begin{equation}
\label{LagFermFCCCs}
\begin{aligned}
...
\end{equation}
%
\Joao{Usa o ambiente \texttt{aligned}. Faz o mesmo que o que tens mas põe o número das equações direito e não precisas de colocar \texttt{\char`\\ nonumber}}
...
%
\Joaorep{There for}{Therefore}, we define the Cabibbo-Kobayashi-Maskawa (CKM) matrix, as $V_{CKM} = U^u_L U^{u ^\dagger }_R $ and write the sensitive terms,
%
\begin{equation}
\mathcal{L}_{kin} \supset \frac{1}{\sqrt{2}} g \overline{u}^\prime_L \gamma^\mu V_{CKM} d_L^\prime W^+_\mu + \text{H.c.} 
\end{equation}
%
The CKM matrix, is a $3 \times 3$ unitary matrix. It is a parametrization of the three mixing angles and \Joaoadd{a} CP-violating KM \Joaoadd{(Kobayashi-Maskawa)} phase. There are many possible conventions to represent the CKM matrix.
%
