...
%
Note that the basis for a BGL like treatment of the 3HDM will be the inclusion of this flavour symmetry, as to attempt to constrain the otherwise dangerous tree-level flavour violating decays that as mentioned in \Joaoadd{Chapter \ref{Chap:SM}} surge in these kind of frameworks. 
...
Note a feature of these additional Higgs doublets is that they do not alter the tree-level EW $\rho$ parameter. 
%
Although, this value can vary by around 20\% from its SM prediction leaving some room for somewhat more exotically manufactured models.  { \color{blue} Explained in the Pdg particle review chapter 11 electroweak limits for new physics, how do I cite that? }. \Joao{Poe a referencia do PDG e indica a pagina e capítulo. Exemplo: As seen in chapter 11, page lala of [AA], }
...
%
Furthermore for completeness we also implemented a \texttt{MadGraph5\_aMC@NLO 2.6.2} routine as to probe future detection of these light scalars in newer collider experiments and discovered \Joaorep{their}{there} is still regions that are far from discovery.  
...
However, it was the 2HDM that quickly became a very popular model after \Joaorep{it's}{its} proposal. 
%
...
%
This endeavour is justified trough phenomenological comparison of the 3HDM to the 2HDM model rather than \Joaoout{simply} the "naturalist" family argument proposed by Weinberg.
%
...
%
\Joaorep{There-for}{Therefore} several mechanisms have been proposed to deal with suppression of these tree-level FCNCs as to allow for richer physics and lighter scalars. 

First, in, \cite{Ferreira_2011,Nebot_2015,ferreira2019strong}, it is proposed a framework in which we have the balancing of CP-odd and CP-even contribution\Joaoadd{s} to FCNCs, however, this requires some fine-tuning, making it very unappealing based on a \Joaoadd{``naturalist''} argument. \Joao{A mim, só me aparece até à referência 97. Estas aqui não me aparecem no fim. Há um bug qualquer. Mesmo assim, tens uma quantidade enorme de referências. Ou tiras algumas, ou o tens tem de ser reduzido.}
% 
...
%
In our analysis th\Joaoadd{e}se will consist of meson decays or oscillations that include the changing of quark flavour within the meson. 
%
These QFV observables will \Joaoout{logically} be compared to measured amplitudes \Joaoadd{i}n collider experiments as to offer a novel exclusion to our model. 
...
Recall that, for the given scalar potential $V$ as seen in \Joaoadd{Eq.~\eqref{eq:3HDM_Scalar_Pot}} to have a stable vacuum it needs to satisfy the tadpole equations i.e. a unchanging potential ensuring that there is a absolute minimum of energy. 
%
...
%
In our analysis of the 3HDM we did not perform the conventional direct diagonalisation of the scalar and Yukawa bi-linear terms expressed in the scalar potential (\Joaoadd{Eq.\,\eqref{eq:3HDM_Scalar_Pot}}) expanded around the vacuum state.
%
Instead we use a intermediate basis in between the process of expressing the mass eigenstates as to significantly simplifies the analysis of the scalar, pseudoscalar, charged scalar and \Joaoadd{q}uark\Joaoout{s} physical terms\Joaoadd{.} 
%
...
%
e.g this base change will allow us to express the charge and pseudoscalar states in such a way as to become apparent that a \Joaoadd{G}oldstone emerges in each case. 
%
These \Joaoadd{G}oldstones will account for the longitudinal polarization of W and Z bosons. 

The transformation from the gauge to the Higgs basis reads, \Joao{Alterei um pouco a matriz. Os elementos estavam muito pequenos}
\begin{equation}
\mathcal{O} = 
\begin{pmatrix}
...
\end{equation}
%
where $v_{13}=\sqrt{v_1^2 + v_3^2}$. This basis transformation explicitly relate\Joaoadd{s} the fields in the following manner, 
%
\begin{equation}
\begin{pmatrix}
\phi_1 \\
...
\end{equation}
%
This is a very important condition as it will reflect what range of values \Joaoadd{that} are available for our Higgs Doublets to take.

Recall that additional \Joaoout{the} gauge couplings to the Higgs bosons have also been precisely measured \Joaorep{making this the only}{meaning the only} way to achieve the correct Higgs kinetic constraints and weak interactions is to a have $h$ state that corresponds to the observed Higgs state with the measured 125.09 GeV mass. 
%
...
Note that strict alignment is not manifestly required by the data and in fact there are ways we could deviate as much as \Joaoadd{$\mathcal{O}(10\%$) (see Ref.\,\cite{Aad_2020})} from the SM predicted gauge coupling value t\Joaoadd{h}rough light mixing. 

This \Joaoout{is} means we are ignoring some of the parameter space by imposing precise Higgs Aligment.
%
...
%
Given time investment, a future version of the presented algorithm (possibly made "smarter" trough new techniques applied in the field such as ML {\color{red}{citar Pino ou Felipe}} \Joao{I approve this message :).}) could efficiently observe and constrict EW effects coming from a more complex mixture of scalars and find more viable regions of the parameter space.  

\subsection{The Yukawa sector}

Now having \Joaorep{delt}{dealt} with the scalar and gauge sector we can continue to study the Yukawa sector. 
%
...
%
Furthermore, examining the effects the imposed symmetry, $\mathrm{U}(1)\times\mathbb{Z}_2$, had on the at the shape of the Yukawa matrices leads us to discover their texture. \Joao{Alterei um pouco para ocupar menos espaço}
%
\begin{equation}
\begin{aligned}
...
\end{equation}
%
Note that there is no overlap between matrix elements inside the up or down sectors, e.g for the case of the $(m_p)_{31}$ and $(m_p)_{32}$ only the first matrix, $\Gamma_1$ contributes \Joaorep{for it's}{to its} value, 
\begin{equation}
...
\end{gathered} 
\end{equation}
We introduce a complex phase in $V^p_{L,R}$ so that these matrices can be parametrized with two angles \Joaoadd{w}hile the parametrization of $U^n_{L,R}$ will require 3. We know these to be the physical degrees of freedom that will be included in the CKM matrix. The CKM matrix elements can then be expressed as, 
\begin{equation}
\begin{gathered}
...
%\end{equation}
%
\Joaoout{where the physical quark mass forms and fields are redefined to include the unitarity transformations.} We can write that $U_L^\dagger = V^\dagger_{\text{CKM}} V_L^\dagger$, which together with,
\begin{equation}
\begin{split}
...
\end{equation} 
%
\Joaoout{These equations} allow for a system of coupled linear equations that can be solved with respect to the Yukawa textures and return their values for a given set of Unitary matrices, CKM matrix and quark masses. 
%
...
% 
\Joaoout{Although given it's relative simplicity we will not present explicitly it's solutions.} 
...
\end{equation}
%
\Joaoadd{w}here we can replace the result seen in \Joaoadd{Eq.~\eqref{eq:3HDM_Quark_gauge_mass}}, 
%
\begin{equation}
\mathcal{L}_Y^{h^\prime} = \frac{h^\prime}{v} \overline{n}_L M_n n_R +  \overline{p}_L M_p p_R 
\end{equation} 
\Joaoadd{s}howing that the SM like Higgs in our model has the same type of tree-level coupling as in the SM. 
%
...
\end{split} 
\end{equation}
To \Joaorep{simply}{simplify} the expression of these interaction terms we go back to the textures of the Yukawas, from the block structures presented, and by virtue of our choice to keep the third row of $U_L$ the same as the \text{CKM} matrix, 
\begin{equation}
(U_L)_{3j} = V_{3j} 
\end{equation}
...
\end{pmatrix}\Joaoadd{,}
\end{equation}
\Joaoadd{w}e can in the down sector Yukawa matrices write 
\begin{equation}
...
\end{equation}
%
We can see trough the proper expansion of these terms that all non diagonal terms (where $j \neq i$) of  $N_{d\,2}$, contain  one CKM matrix term. While for  $N_{d\,1}$ we have two CKM matrix \Joaorep{terms}{elements}. Even given these terms we must suppress the size of the elements in $\Gamma_1$. 
...
%
It can however be expressed in a block diagonal matrix trough the rotation matrix we introduced in \Joaoadd{Eq.~\eqref{eq:3HDM_Orthg}}, thus in the Higgs \Joaoadd{b}asis our mass matrix would take the shape of, %  and a new orthogonal rotation matrix, $\mathcal{O}^T$,
%
\begin{equation}
...
\end{pmatrix} \Joaoadd{.}
\end{equation} 
Therefor\Joaoadd{e} the mass eigenstates of the pseudoscalars in the mass basis can be related to the gauge basis by the equation,
\begin{equation}
\label{Rot_1_3HDM}
...
\end{pmatrix} \Joaoadd{.}
\end{equation}
Expanding the system seen in \Joaoadd{Eq.\,\eqref{Rot_1_3HDM}}, we can deduce the equations for the mass terms in terms of the original Lagrangian parameters, 
\begin{equation}
...
\end{gathered}
\end{equation}
From these two sets of equations (\Joaoadd{Eqs.~\eqref{eq:3HDM_Pseudo_relation} and \eqref{eq:3HDMP_pseudo_intermedium}}) we can show that, % we can show that,
\begin{equation}
\begin{split}
...
\end{equation} 
%
T\Joaoadd{h}rough the inversion of these equations we can obtain a parametrization of, $\lambda_{10}$, $v$, trough the mixing angles $\psi_{1,2}$ VEV magnitude, $v$, and the new pseudacalar mixing angles $\gamma_{1,2}$, as seen in,
%
\begin{equation}
\begin{split}
...
\end{equation}
\Joaoadd{th}rough the same rotation to the Higgs basis we produce a similar structure to the mass matrix of the pseudoscalar fields as seen in,  
%
...
\end{equation}
%
\Joaoadd{wh}ere we observe again that a line and column of zeros ensures that we have a Goldstone boson. Like in the previous example this will be incorporated in the $W^\pm$ Gauge Boson. The terms in this matrix are written as,
%
...
\end{pmatrix}
\end{equation}
\Joaoadd{r}elating to the massive states as 
\begin{equation}
...
\end{equation}
%
\Joaoadd{w}here we see the explicit mass eigenstates of the charged scalars $H^\pm_1$ and $H^\pm_2$. 
%
...
\end{split} 
\end{equation}
Notice how the charged eigenstates are dependent only of 3 Lagrangian parameters. For brevety we do not show the mass terms explicitly but given we have three equations we can \Joaorep{parmaaterize}{parameterize} another 3 three parameters of the Lagrangian, $\lambda_{7-9}$ in terms of physical masses, VEVs and a mixing angle, as seen, 
\begin{equation}
\begin{split}
...
%
As stated, \Joaorep{as}{at} the starting point of our numerical scans we must ensure a positively defined spectrum i.e. no tachyonic solutions and a stable vacuum (at least at tree-level) t\Joaoadd{h}rough tadpole equations. % from the very beggining.  
% 
...
% 
Then \Joaoadd{to} obtain relevant data in a timely fashion we performed a inversion process as to scan over physical parameters instead of Lagrangian terms.  
% 
...
%
\Joaorep{And in fact although}{Although} SPheno utilizes \Joaoout{it's  calculations} tree-level masses for all radiative processes ( like EW observables) and calculates them correctly just with tree-level masses replaced for loop-level masses, \Joaoout{Still} it \Joaoadd{it is still} relevant to admit that we must \Joaorep{assume}{consider} that the radiative corrections are subdominant as for our results to have any significant validity.
%
...
% 
The exercise of proper inversion \Joaorep{with higher than tree-level order must be left as future work}{for higher orders is left for future work}.

The ranges of accepted values within our scans are seen in the following Table.
%
\begin{table}[H]
\centering
\begin{tabular}{ccc}
...
% 
This required another set of linear equations to be solved, within experimental error bars of all quark and lepton masses\Joaoout{, in order to the yukawa couplings}.
% 
...
%
Introduced to enable our simulations to use the existence of quark flavour violation (QFV) processes to constrict our parameter space t\Joaoadd{h}rough \Joaoout{the} Wilson coefficients calculated by SPheno. 
...
%
In fact what we expected to find is a light enhancement of processes already included in the SM t\Joaoadd{h}rough the exotic additions of scalars along side the suppression mechanisms we discussed above. 
...
For the most relevant observables the following table shows \Joaoout{values} experimental values and error bars along with SM predicted values and theoretical uncertainties, 
%
\begin{table}[H]
\centering
...
To briefly introduce how these QFV observables relate to NP we show some representative diagrams, \Joaoadd{as shown in Figs.~\ref{3HDM_Leptonics}, \ref{fig:3HDM_oscilation_meson} and  \ref{fig:3HDMChiSGamma}}). \Joaoout{At} \Joaoadd{T}ree-level processes that include $H^\pm_{1,2}$ lead to leptonic and semileptonic (a decay in which one lepton and the corresponding neutrino is produced in addition to one or more hadrons) \Joaorep{processes}{decay chains},  
%
\begin{figure}[H]
\centering
\vspace{0.2cm}
...
\end{fmffile}}
\vspace{0.9cm}
\caption{Leptonic and semi-leptonic $H^\pm$ tree-level diagrams where M stands for a generic meson. \Joaoadd{$\ell$ is any lepton with $\nu_\ell$ being the corresponding neutrino.}}
...
%     \begin{minipage}{.5\textwidth}
%         \centering
%         \includegraphics[width=.99\textwidth]{Temporary.png}
%     \end{minipage}
%     %
...
% \end{figure}
%
\Joaoout{where  $\ell$ is any lepton.} Again, note that our model is lepton flavour universal\Joaoadd{, meaning that} all generations are included in these diagrams and the neutrino always corresponds to the same lepton generation. 
%Note that in our model is lepton flavour universal so 
%
In Fig.\,\ref{3HDM_Leptonics} we see how a virtual $H^\pm$ leads to \Joaoout{more} meson decay into leptons. 
%
...
% \end{figure}
%
Still at loop-level, we have the final channel, $\chi_s \gamma$. The quark level transition occurring here is, $b \rightarrow s \gamma$ trough the annihilation of a quark and anti-quark. We show the Feynman diagrams where this process happen \Joaoadd{in Fig.~\ref{fig:3HDMChiSGamma}.}
%
\begin{figure}[H]
\centering
\vspace{0.3cm}
...
\vspace{0.2cm}
\caption{NP contribution to the transition mediated by a charged or neutral Higgs at \Joaoadd{one-}loop order as to contribute to $\mathcal{BR} (B \rightarrow \chi_s \gamma)$ trough $b \rightarrow \bar{s} \gamma$ }
...
%
In the 3HDM, looking at \Joaoadd{Eq.~\eqref{eq:3HDM_Scalar_Pot}}, we can see that it is not a simple process. This check begins by ensuring the couplings seen in \Joaoadd{Eq.\,\eqref{Unbound_Stage_1}} are positive so that the potential does not tend towards $-\infty$ when the fields become large. 
...
%
With it we find the parameter space that not only respects all scalar and EW analysis as implemented on the B-L-SM model, but also keeps all examined QFV observables within the 2 $\sigma$ deviation limit and ensures the proper \Joaorep{invesion}{inversion} process. 
...
%
We can see the S, T and U variables for our parameter space in the \Joaoout{following} \Joaoadd{Figs.~\ref{Fig:3HDM_STU}}, 
%
\begin{figure}[H]
	\centering
...
\begin{figure}[H]\ContinuedFloat
	...
	\caption{\Joao{As labels dentro dos plots estão muito pequenas. Não dá para ler. Para algumas imagens mais abaixo também é a mesma coisa.}}
	\label{Fig:3HDM_STU}
\end{figure}
%
...
%
As in the B-L-SM the values for $(\Delta S , \Delta T , \Delta U )$ are taken from  \Joaoout{Ref}\,\cite{Baak_2012}.
...
\begin{figure}[H]
	...
	\includegraphics[width=.49\textwidth]{Images/3HDM/PT_Folder/Bsmumu_H1.pdf}	\includegraphics[width=.49\textwidth]{Images/3HDM/PT_Folder/Delta_Md_H1.pdf}
